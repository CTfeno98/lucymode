...
exports.run = async(client, message, args) => {
    ...
    let sebep = args.slice(1).join(' ') || "Belirtilmemiş."
     if(!user) return message.channel.send(':exclamation:  ``Bir kişi etiketlemelisin.``')
     if(user.id === message.author.id) return message.channel.send(':exclamation:  ``Kendini Kickleyemezsin``')
     if(user.id === client.user.id) return message.channel.send(':exclamation:  ``Botu Kickleyemezsin``')
  if(user.id === message.guild.ownerID) return message.channel.send(':exclamation:  ``Sunucu sahibini Kickleyemessin Zaten Yetkim Yetmez :)``')
    if (!message.guild.member(user).bannable) return message.reply(':exclamation:  ``Bu kişinin rolü senden üstte veya `Üyeleri yasakla` yetkisine sahip.``');
